...
def TrackImages():
    ...
    if exists:
        ...
        with open("Login\Login_" + date + ".csv", 'a+') as csvFile1:
            ...
            writer.writerow(login)
        csvFile1.close()
    with open("Login\Login_" + date + ".csv", 'r') as csvFile1:
        reader1 = csv.reader(csvFile1)
...
def halaman2():
    ...
    frame.place(relx=0.32, rely=0.15, relwidth=0.39, relheight=0.80)
    
    fr_head = tkinter.Label(frame, text="Register", fg="white",bg="black" ,font=('times', 17, ' bold ') )
    fr_head.place(x=0,y=0,relwidth=1)
